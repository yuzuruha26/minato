...
export const CatStatus: React.FC<CatStatusProps> = ({ point, onUpdatePoint, cats, onUpdateCat, onAddCat, isWatered, onToggleWater, onBack, onHome, currentUser, points, zones }) => {
  ...
  const handleSaveReport = async () => {
    ...
    // Firebase (またはローカル) に保存
    await dataStorage.saveReport(fullReport);

    // 2. ネコ本体の最終給餌日時とステータスを更新
  ...
  const startEditing = () => {
    if (selectedCat && currentUser.role === 'admin') {
      ...
      setFeatureParts(parseFeatures(selectedCat.features));
      setIsEditing(true);
  ...
  const parseCoordinates = (input: string) => {
    ...
    if (matches.length >= 2) {
      try {
        ...
        setPreviewCoords({ lat: coords[0], lng: coords[1] });
        return;
      } catch (e) {}
    }
          ...
          <div className="relative h-72 md:h-96 w-full bg-gray-200 group">
            ...
            <div className="absolute top-0 inset-x-0 p-4 flex justify-between items-start z-20">
              <div className="flex gap-2">
                <button onClick={() => isEditing ? cancelEditing() : setSelectedCat(null)} className="p-2 bg-black/30 backdrop-blur-md rounded-full text-white hover:bg-black/50 transition-colors">
                  {isEditing ? <X size={20} /> : <ArrowLeft size={20} />}
                </button>
                {!isEditing && (
                  <button onClick={onHome} className="p-2 bg-black/30 backdrop-blur-md rounded-full text-white hover:bg-black/50 transition-colors">
                    <Home size={20} />
                  </button>
                )}
              </div>
              {!isEditing && currentUser.role === 'admin' && (
                <button onClick={startEditing} className="p-2 bg-white/90 backdrop-blur-md rounded-full text-gray-700 hover:bg-white transition-colors shadow-sm">
                  <Pencil size={20} />
                </button>
              )}
              {isEditing && (
                <button onClick={saveEditing} className="px-4 py-2 bg-orange-500 rounded-full text-white font-bold hover:bg-orange-600 transition-colors shadow-sm text-sm">
                  保存
                </button>
              )}
            </div>
            {isEditing && (
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-10">
                <label className="cursor-pointer flex flex-col items-center gap-2 text-white p-4 rounded-xl hover:bg-white/10 transition-colors">
                  <div className="p-3 bg-white/20 rounded-full backdrop-blur-sm"><ImagePlus size={32} /></div>
                  ...
                  <div className="bg-white p-5 rounded-xl shadow-sm border border-orange-200">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input type="text" value={editData.name} onChange={(e) => setEditData({...editData, name: e.target.value})} placeholder="例: タマ (3号)" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-orange-400 font-bold text-lg" />
                      </div>
                      <div className="p-4 bg-blue-50 rounded-lg border border-blue-100 space-y-3">
                         ...
                         {[0, 1, 2, 3].map((index) => {
                             ...
                             return (
                               <div key={index}>
                                 <label className="block text-[10px] text-gray-500 font-bold mb-1">{index === 0 ? '優先度1 (メイン)' : `優先度${index + 1}`}</label>
                                 <select value={priorityPoints[index]} onChange={(e) => handlePriorityPointChange(index, e.target.value)} className={`w-full p-2 bg-white border rounded-lg text-sm ${index === 0 ? 'border-orange-300 font-bold text-gray-800' : 'border-gray-200 text-gray-600'}`}>
                                   ...
                                   {zones.map(z => {
                                      ...
                                      return (
                                        <optgroup key={z.id} label={z.name}>
                                           {zonePoints.map(p => !higherPrioritySelections.includes(p.id) && <option key={p.id} value={p.id}>{z.name} {p.name}</option>)}
                                        </optgroup>
                                      );
                                   })}
                                 </select>
                      ...
                      <div className="p-4 bg-orange-50 rounded-lg border border-orange-100 space-y-3">
                         ...
                         <div className="grid grid-cols-2 gap-3">
                            <div>
                               ...
                               <select value={featureParts.color} onChange={(e) => setFeatureParts({...featureParts, color: e.target.value})} className="w-full p-2 bg-white border border-gray-200 rounded-lg text-sm">
                            ...
                            <div>
                               ...
                               <select value={featureParts.pattern} onChange={(e) => setFeatureParts({...featureParts, pattern: e.target.value})} className="w-full p-2 bg-white border border-gray-200 rounded-lg text-sm">
                                 <option value="">未選択</option>
                            ...
                            <div>
                               ...
                               <select value={featureParts.fur} onChange={(e) => setFeatureParts({...featureParts, fur: e.target.value})} className="w-full p-2 bg-white border border-gray-200 rounded-lg text-sm">
                                 <option value="">未選択</option>
                            ...
                            <div>
                               ...
                               <select value={featureParts.tail} onChange={(e) => setFeatureParts({...featureParts, tail: e.target.value})} className="w-full p-2 bg-white border border-gray-200 rounded-lg text-sm">
                                 <option value="">未選択</option>
                      ...
                      <div>
                        <label className="block text-gray-500 text-xs font-bold mb-1">詳細特徴</label>
                        <textarea value={featureParts.other} onChange={(e) => setFeatureParts({...featureParts, other: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-orange-400 text-sm h-20" />
                      </div>
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {(['healthy', 'injured', 'sick', 'unknown'] as const).map(status => (
                            <button key={status} onClick={() => setEditData({...editData, status})} className={`py-2 px-3 rounded-lg text-sm border-2 transition-all ${editData.status === status ? 'border-orange-500 bg-orange-50 text-orange-700 font-bold' : 'border-gray-100 text-gray-500'}`}>
                              {status === 'healthy' ? '健康' : status === 'injured' ? '怪我' : status === 'sick' ? '病気' : '不明'}
                            </button>
                          ))}
                        </div>
                      </div>
                ...
                <>
                  ...
                  <div className="space-y-6">
                    <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                      ...
                      <button onClick={() => setReportForm({...reportForm, fed: !reportForm.fed})} className={`w-full py-3 px-4 rounded-lg border-2 font-bold flex items-center justify-center gap-2 transition-all ${reportForm.fed ? 'border-orange-500 bg-orange-50 text-orange-600' : 'border-gray-200 text-gray-400'}`}>
                        {reportForm.fed && <Check size={18} />} 餌やり完了
                      </button>
                    </div>
                    <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {[
                          ...
                          <button key={status.id} onClick={() => setReportForm({...reportForm, condition: status.id as any})} className={`flex flex-col items-center justify-center py-4 rounded-lg border-2 transition-all ${reportForm.condition === status.id ? 'border-gray-800 bg-gray-800 text-white' : 'border-gray-100 bg-gray-50 text-gray-500'}`}>
                    ...
                    <div className="bg-red-50 p-5 rounded-xl shadow-sm border border-red-200">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-start gap-4">
                          <label className="cursor-pointer bg-white border border-red-200 text-red-600 p-3 rounded-lg flex flex-col items-center justify-center gap-1 min-w-[80px]">
                            <Camera size={24} /><span className="text-[10px] font-bold">写真追加</span>
                            <input type="file" accept="image/*" className="hidden" onChange={handleUrgentPhotoChange} />
                          </label>
                          {reportForm.urgentPhoto && (
                            <div className="relative h-20 w-20 rounded-lg overflow-hidden border border-red-200">
                              <img src={reportForm.urgentPhoto} alt="Urgent" className="w-full h-full object-cover" />
                              <button onClick={() => setReportForm({...reportForm, urgentPhoto: ''})} className="absolute top-0 right-0 bg-red-600 text-white p-0.5"><X size={12} /></button>
                            </div>
                          )}
                        </div>
                        <textarea className="w-full p-3 bg-white rounded-lg border border-red-200 text-gray-800" rows={2} placeholder="緊急性の高い内容..." value={reportForm.urgentDetail} onChange={(e) => setReportForm({...reportForm, urgentDetail: e.target.value})} />
                      </div>
                    </div>
                    <div className="bg-yellow-50 p-5 rounded-xl shadow-sm border border-yellow-200">
                      <label className="block text-yellow-700 text-sm font-bold mb-3 flex items-center gap-2"><FileWarning size={18} />【注意】</label>
                      <textarea className="w-full p-3 bg-white rounded-lg border border-yellow-200 text-gray-800" rows={2} placeholder="気になった点..." value={reportForm.attentionDetail} onChange={(e) => setReportForm({...reportForm, attentionDetail: e.target.value})} />
                    </div>
                    <button onClick={handleSaveReport} disabled={submitted} className="w-full py-4 bg-orange-500 text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-2">
                      {submitted ? <><Check /> 記録しました</> : <><Save /> ステータスを保存</>}
                    </button>
                  </div>
                </>
              )}
    ...
    <div className="min-h-screen bg-[#fdfbf7] p-4">
      <div className="max-w-5xl mx-auto w-full">
        <div className="sticky top-0 bg-[#fdfbf7]/90 backdrop-blur-sm z-10 py-4 mb-4 border-b border-orange-100 flex items-center gap-3">
          <div className="flex gap-2">
            <button onClick={onBack} className="p-2 hover:bg-orange-100 rounded-full text-orange-600"><ArrowLeft /></button>
            <button onClick={onHome} className="p-2 hover:bg-orange-100 rounded-full text-orange-600"><Home /></button>
          </div>
          <div><h1 className="text-xl font-bold text-gray-800">{point.name}</h1><p className="text-xs text-gray-500">ネコ一覧 ({filteredCats.length}匹)</p></div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-blue-100 mb-6 max-w-2xl mx-auto">
          ...
          <button onClick={onToggleWater} className={`w-full flex items-center justify-between p-4 rounded-lg border-2 transition-all ${isWatered ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-gray-50 border-gray-200 text-gray-400'}`}>
            <span className="font-bold flex items-center gap-2"><Droplets size={20} className={isWatered ? 'fill-blue-600' : ''} />水やり</span>
        ...
        {currentUser.role === 'admin' && onUpdatePoint && (
          <div className="bg-white p-4 rounded-xl shadow-sm border border-orange-200 mb-6 max-w-2xl mx-auto">
            <div className="flex justify-between items-center mb-3">
              <h2 className="text-sm font-bold text-gray-600 flex items-center gap-2"><Settings size={16} className="text-gray-400" />管理者設定</h2>
              <button onClick={() => setIsEditingPoint(!isEditingPoint)} className="text-xs text-orange-500 font-bold">{isEditingPoint ? 'キャンセル' : '編集'}</button>
            </div>
            {isEditingPoint ? (
              <div className="space-y-4 animate-fade-in-up">
                <div><label className="block text-xs text-gray-500 font-bold mb-1">拠点名</label><input type="text" value={editPointName} onChange={(e) => setEditPointName(e.target.value)} className="w-full p-3 bg-gray-50 border rounded-lg text-sm font-bold" /></div>
                <div><label className="block text-xs text-gray-500 font-bold mb-1">座標</label><textarea value={coordInput} onChange={(e) => setCoordInput(e.target.value)} className="w-full p-3 bg-gray-50 border rounded-lg text-sm h-24" placeholder="座標を入力" /></div>
                {coordError && <p className="text-red-500 text-xs mt-1 font-bold">{coordError}</p>}
                <button onClick={handleSavePoint} className="w-full py-3 bg-orange-500 text-white text-sm font-bold rounded-xl">情報を更新</button>
              </div>
            ) : (
              <div className="text-xs text-gray-400 flex items-center gap-2"><MapPin size={12} />{point.lat ? <span className="font-mono">{point.lat.toFixed(6)}, {point.lng?.toFixed(6)}</span> : <span>座標未設定</span>}</div>
            )}
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {filteredCats.map(cat => (
            <div key={cat.id} onClick={() => setSelectedCat(cat)} className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md cursor-pointer group">
              <div className="aspect-square relative overflow-hidden"><img src={cat.imageUrl} alt={cat.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" /><div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/60 to-transparent p-3"><p className="text-white font-bold text-sm truncate">{cat.name}</p></div></div>
              <div className="p-3"><div className="flex items-center gap-2 mb-2"><span className={`w-2 h-2 rounded-full ${cat.status === 'healthy' ? 'bg-green-500' : 'bg-red-500'}`}></span><span className="text-xs text-gray-500">{cat.status === 'healthy' ? '健康' : '要観察'}</span></div><p className="text-xs text-gray-400 line-clamp-1">{cat.features}</p></div>
            </div>
          ))}
          {currentUser.role === 'admin' && (
            <button onClick={startAdding} className="bg-gray-50 rounded-xl overflow-hidden border-2 border-dashed border-gray-300 hover:border-orange-400 hover:bg-orange-50 transition-all cursor-pointer flex flex-col items-center justify-center min-h-[160px] text-gray-400 hover:text-orange-500 group">
              <div className="p-3 bg-white rounded-full shadow-sm mb-2 group-hover:scale-110 transition-transform"><Plus size={24} /></div>
